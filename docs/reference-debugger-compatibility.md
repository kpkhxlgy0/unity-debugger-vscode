...
| EVAL-02 | Hover/Watch `RuntimeInitializeLoadType.AfterSceneLoad` | Hover and Watch both report <code>The identifier `RuntimeInitializeLoadType` is not in the scope</code>; whether Hover also raises a notification is not yet explicitly recorded | Hover and Watch report the same scope diagnostic; Hover also raises a bottom-right error notification | value/diagnostic aligned; notification presentation unverified | 2026-08-06 MyGame reference and Pure 0.3.0 runs | Re-run the identical Hover with the reference debugger before accepting or replacing the notification behavior |
...
- Hover and Watch for `RuntimeInitializeLoadType.AfterSceneLoad` both reported
  ...
  in a bottom-right Hover error notification. The earlier reference run did not explicitly record whether that
  notification appeared, so only the value/diagnostic portion of EVAL-02 is aligned pending an exact UI A/B.
