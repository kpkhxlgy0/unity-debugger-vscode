...
| EX-01 | Caught/unhandled exception filters | Exposes unchecked `All Exceptions` and `User-Unhandled Exceptions`; default Play does not pause; User-Unhandled alone does not stop for the handled `TaskCanceledException`; enabling All stops at its throw site in `UnityAssetLinkReceiver.cs:94` with the yellow marker and inline exception panel; one Continue completes the Play exit | Build `0.3.0+g6bbcf11aab44` reaches line 94, but shows only `Exception has occurred.`, lacks the expected yellow marker, and immediately stops again after Continue instead of completing the exit | divergent; replacement in progress | 2026-08-06 MyGame reference runs; 2026-08-10 Pure screenshot and Adapter log | Keep the reference behavior; remove stop-time expression evaluation and use non-invoking runtime exception metadata |
