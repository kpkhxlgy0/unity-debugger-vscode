...
| CTRL-02b | Jump to Cursor and Set Next Statement | Jump to Cursor stops at line 1076; Set Next Statement immediately moves the yellow marker to line 1076; no warning or marker loss, and subsequent Step Over works | Installed build throws `error while processing request 'goto' (exception type: NotSupportedException)`; replacement source now resolves a real code location and uses the requested thread for the reference SetIP/one-shot-breakpoint sequence | divergent runtime; replacement pending A/B | 2026-08-06 MyGame reference run; 2026-08-07 Pure run, Adapter log, and full reference IL inspection | User confirmed replacing the complete Goto control path; do not mark aligned until the replacement package passes MyGame |
