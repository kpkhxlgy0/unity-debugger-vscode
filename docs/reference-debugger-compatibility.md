...
| VAR-02 | Instance Locals topology | Locals has peer synthetic entries `Active scene`, `this`, and `this.gameObject`; `transform` is below `this.gameObject`; Getter values display directly with no `Properties` group | Installed build `0.3.0+g22c0cbfa14a6` shows the three peer entries but fails when `this.gameObject` expands; the approved source replacement now binds synthetic values to their stopped frame and awaits packaging/retest | divergent; replacement pending verification | 2026-08-06 MyGame reference/Pure runs, screenshot, and focused source tests | User approved replacing the synthetic GameObject child source; retest before changing status |
| VAR-03 | Expand `this` member hierarchy | Reference preserves a `base` node instead of flattening inherited members | Installed build does not reproduce the `base` hierarchy and omits the source-defined `CurrentSnapshot` Getter from the visible tree; the approved source replacement preserves base types and inlines non-public members but is not yet installed | divergent; replacement pending verification | 2026-08-06 MyGame user A/B observation, source, and focused tests | User approved replacing the flattened member presentation; exact remaining member placement still requires the matched real run |
...
| EVAL-03 | Locals, Getter, `ToString()`, collection expansion | Getter values display directly; `this.ToString()` and `gameObject.ToString()` return Unity object display strings; `BoardIndexByCell` displays as `int[5,8]` and expands normally; no loading placeholder or delayed update was observed in this run | At the matched instance stop, top-level synthetic values display, but expanding `this.gameObject` returns a null-reference child; Getter grouping and the remaining Watch/collection checks are not yet verified in this build | divergent; reference verified | 2026-08-06 MyGame reference run and Pure screenshot | Stop for user decision on replacing the synthetic child source; do not mark untested inspection paths aligned |
