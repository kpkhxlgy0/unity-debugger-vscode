...
| CTRL-02b | Jump to Cursor and Set Next Statement | Jump to Cursor stops at line 1076; Set Next Statement immediately moves the yellow marker to line 1076; no warning or marker loss, and subsequent Step Over works | Build `0.3.0+g4869d3b8cd91` returns no targets and VS Code reports `No executable code is associated at the current cursor position`; replacement source now also ignores the cursor column like the reference | divergent runtime; replacement pending A/B | 2026-08-06 MyGame reference run; 2026-08-07 Pure runs, Adapter logs, and full reference IL inspection | User confirmed replacing the complete Goto control path; do not mark aligned until the replacement package passes MyGame |
