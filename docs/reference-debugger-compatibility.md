...
| CTRL-02 | Step-in Targets and Goto capability/presentation | All three commands are visible and enabled; Step Into Target enters `EnsureStyles()` at line 1070, Jump to Cursor stops at line 1076, and Set Next Statement immediately moves the yellow marker to line 1076; no warning or marker loss, and subsequent Step Over works | Installed build `0.3.0+gaaca01c722f7` still stopped without entering `EnsureStyles()`; its log confirms target enumeration and targeted Step In were dispatched, followed by a new stop about 0.3 seconds later | divergent; replacement pending A/B | 2026-08-06 MyGame reference run, 2026-08-07 Pure runs, current Adapter log, reference binary inspection, MyGame IL/PDB inspection, and source-candidate tests | Reference creates an unfiltered breakpoint request and checks request identity plus thread ID when dispatching the event; the failed candidate incorrectly set the runtime request's Thread filter. The next source candidate removes that filter and mirrors dispatch-time matching; do not mark aligned before MyGame verification |
