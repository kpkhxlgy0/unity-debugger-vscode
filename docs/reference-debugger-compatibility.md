...
| VAR-02 | Instance Locals topology | Locals has peer synthetic entries `Active scene`, `this`, and `this.gameObject`; `transform` is below `this.gameObject`; Getter values display directly with no `Properties` group | Build `0.3.0+g22c0cbfa14a6` now shows the three peer entries, but expanding `this.gameObject` produces `Object reference not set to an instance of an object.` instead of members | divergent; top level aligned, expansion divergent | 2026-08-06 MyGame reference/Pure runs and screenshot | Stop for user decision before replacing the current synthetic GameObject child source |
| VAR-03 | Expand `this` member hierarchy | Reference preserves a `base` node instead of flattening inherited members | Pure does not reproduce the reference `base` hierarchy, and the source-defined `CurrentSnapshot` Getter is absent from the visible member tree | divergent; reference verified for `base` | 2026-08-06 MyGame user A/B observation and `GamePrototypeRuntime.cs:102` source | Stop for user decision before replacing the current flattened member presentation |
